...
/**
 * TabHoraires — habillage éditorial (.adm-vit), comme les autres onglets.
 *
 * Le tableau perd ses cadres : filets fins entre les lignes, nom du jour en
 * Fraunces, heures soulignées plutôt qu'encadrées. Toutes ces règles existent
 * déjà dans admin.css sous « ══ HORAIRES ══ », scopées `.contenu.adm-vit` —
 * elles étaient dormantes parce que ce composant était resté sur l'habillage
 * d'interface. Trois classes leur répondent ici : `hr-ferme` (la ligne d'un
 * jour fermé s'éteint sans disparaître), `hr-mention` (« Fermé »), `hr-vide`
 * (« pas de service » à côté d'un créneau laissé vide).
 */
export default function TabHoraires() {
  ...
  /** Un service : deux heures soulignées, ou la mention qui explique le vide. */
  function Service({ h, ouverture, fermeture }: {
    h: OpeningHour;
    ouverture: "lunch_open" | "dinner_open";
    fermeture: "lunch_close" | "dinner_close";
  }) {
    if (h.is_closed) return <span className="hr-mention">Fermé</span>;
    const vide = !h[ouverture] && !h[fermeture];
    return (
      <div className="creneau-edit">
        <input type="time" defaultValue={h[ouverture] || ""} onBlur={(e) => setHeure(h, ouverture, e.target.value)} />
        <span>→</span>
        <input type="time" defaultValue={h[fermeture] || ""} onBlur={(e) => setHeure(h, fermeture, e.target.value)} />
        {/* Un créneau vide ne dit rien de lui-même : on l'écrit, plutôt que de
            laisser deux champs muets qu'on croit mal remplis. */}
        {vide && <span className="hr-vide">pas de service</span>}
      </div>
    );
  }
  ...
  return (
    <>
      <div className="topbar adm-vit"><div>
        <span className="adm-vit-eyebrow">Pilotage</span>
        <h1>Horaires</h1>
        <div className="sous">Ouvertures affichées sur le site et fermetures exceptionnelles</div>
      </div></div>
      <div className="contenu adm-vit">
        ...
        <div className="bloc">
          ...
          <table className="tab-horaires tbl-cartes"><thead><tr>
            ...
            {jours.map((h) => (
              <tr key={h.id} className={h.is_closed ? "hr-ferme" : undefined}>
                ...
                <td data-label="Midi"><Service h={h} ouverture="lunch_open" fermeture="lunch_close" /></td>
                <td data-label="Soir"><Service h={h} ouverture="dinner_open" fermeture="dinner_close" /></td>
              </tr>
            ))}
          </tbody></table>
            ...
            <table className="tab-fermetures tbl-cartes"><thead><tr><th>Période</th><th>Service</th><th>Motif</th><th>Note interne</th><th></th></tr></thead><tbody>
              {cp.rows.map((cl) => (
                <tr key={cl.id}>
                  ...
                  <td className="td-actions"><button className="adm-vit-lien danger" onClick={() => cp.remove(cl.id)}>Supprimer</button></td>
                </tr>
              ))}
            </tbody></table>
          )}
