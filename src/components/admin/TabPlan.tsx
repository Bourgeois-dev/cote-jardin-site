...
// Composant partagé (fond identique) : rend un plateau + N chaises réparties
// autour, selon la capacité et la forme. Piloté uniquement par les props.
function TableSVG({ size, capacity, round }: { size: number; capacity: number; round: boolean }) {
  const c = size / 2;
  ...
  return (
    <svg className="tp-table-svg" viewBox={`0 0 ${size} ${size}`} width={size} height={size} aria-hidden="true">
      {chairs}
      {round
